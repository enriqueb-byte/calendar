(function () {
  ...
  /** Effective expectancy: custom override if active and valid, else default by gender. */
  ...
  /** Current age in full years from DOB string. */
  ...
  /** Youngest child DOB (latest date in lifeChildren) or null. */
  ...
  /** Summers (years) of shared mobility with parents: min over parents of years until that parent turns 80. Returns null if no valid parents. */
  ...
  /** Number of calendar months lived since DOB (0-based index of current month). */
  ...
  /** Months remaining from now to expected end of life (by gender). */
  ...
  /** Updates the 4 Seasons reporting areas. */
  function renderSeasonsDashboard() {
    ...
    // Classify each season: 'active' (now), 'onTheWay' (future), 'inactive' (past or N/A)
    var peakStatus = age == null ? 'inactive' : age < 25 ? 'onTheWay' : age <= 55 ? 'active' : 'inactive';
    var superheroStatus = !youngestChildDob ? 'onTheWay' : childAgeYears == null ? 'inactive' : childAgeYears < 12 ? 'active' : 'inactive';
    var explorerStatus = age == null ? 'inactive' : age < 70 ? 'active' : 'inactive'; // no "on the way" (birth–70)
    ...
    if (multiGenEl || multiGenVal) {
      if (!state.lifeHasLivingParents) {
        setSeasonRow(multiGenEl, multiGenVal, 'Check "I have living parents" in Facts and add birth dates.', '—');
        ...
        setSeasonRow(multiGenEl, multiGenVal, 'Shared-mobility window (to 80) complete for parents.', '—');
      } else {
    ...
    // Category blocks only in Active section: Vitality (0,5,2), Connection (1,4), Growth (6,3)
    ...
    var onTheWayContent = document.getElementById('seasonsOnTheWayContent');
    ...
    var onTheWaySection = document.getElementById('seasonsOnTheWaySection');
    ...
    var onTheWayLabel = document.getElementById('seasonsOnTheWayLabel');
    var activeCount = 0;
    var onTheWayCount = 0;
    var inactiveCount = 0;
    var vitalityOrder = [0, 5, 2];   // Peak Output, Joint Health (HighImpact), Explorer Window
    var connectionOrder = [1, 4];    // Superhero Phase, Multi-Gen Window
    var growthOrder = [6, 3];        // Discovery (NeuroPlastic), Wisdom Transition
    var i, card, status;
    for (i = 0; i < cards.length; i++) {
      ...
      if (status === 'active') {
        ...
        card.classList.add('season-on-the-way');
        if (onTheWayContent) onTheWayContent.appendChild(card);
        onTheWayCount++;
      } else {
        card.classList.add('season-inactive');
    ...
    if (onTheWaySection) {
      onTheWaySection.classList.toggle('hidden', onTheWayCount === 0);
      if (onTheWayLabel) onTheWayLabel.textContent = onTheWayCount > 0 ? 'On the Way (' + onTheWayCount + ')' : 'On the Way';
    }
    if (inactiveSection) {
      inactiveSection.classList.toggle('hidden', inactiveCount === 0);
      if (inactiveLabel) inactiveLabel.textContent = inactiveCount > 0 ? 'Inactive (' + inactiveCount + ')' : 'Inactive';
    }
  }

  /** Renders the list of child birthday rows in Facts. */
  ...
  /** Renders the list of parent birthday rows in Facts. */
  ...
  /** Renders the list of milestone rows in Reflect (sorted by date, most recent first). */
  function renderLifeMilestones() {
    ...
    for (var i = 0; i < sorted.length; i++) {
      ...
      var safeDate = escapeHtml(dateVal);
      html.push(
        '<li class="flex flex-wrap items-center gap-2" data-milestone-id="' + escapeHtml(id) + '">' +
        ...
        '<button type="button" class="life-remove-milestone text-xs text-ink-500 hover:text-red-600 px-2 py-1 rounded" data-milestone-id="' + escapeHtml(id) + '" aria-label="Remove milestone">Remove</button>' +
        '</li>'
      );
    }
    listEl.innerHTML = html.join('');
  ...
  /** Returns month/year string for the given life month index (for tooltip). */
  ...
  function renderLifeDashboard() {
    var dobInput = document.getElementById('lifeDateOfBirth');
    var genderSelect = document.getElementById('lifeGender');
    var clockEl = document.getElementById('lifeClock');
    var scarcityEl = document.getElementById('lifeScarcityNote');
    if (dobInput) dobInput.value = state.lifeDateOfBirth || '';
    if (genderSelect) genderSelect.value = state.lifeGender || 'male';
    var expectancyInput = document.getElementById('lifeExpectancyInput');
    var expectancyOverrideCheck = document.getElementById('lifeExpectancyOverrideCheck');
    if (expectancyInput) {
      var active = state.lifeExpectancyOverrideActive;
      expectancyInput.disabled = !active;
      if (active) {
        expectancyInput.value = state.lifeExpectancyOverride != null ? String(state.lifeExpectancyOverride) : '';
      } else {
        expectancyInput.value = String(getLifeExpectancyYears(state.lifeGender || 'male'));
      }
    }
    if (expectancyOverrideCheck) expectancyOverrideCheck.checked = state.lifeExpectancyOverrideActive;
    var hasChildrenCheck = document.getElementById('lifeHasChildrenCheck');
    var childrenBlock = document.getElementById('lifeChildrenBlock');
    if (hasChildrenCheck) hasChildrenCheck.checked = state.lifeHasChildren;
    if (childrenBlock) childrenBlock.classList.toggle('hidden', !state.lifeHasChildren);
    var hasLivingParentsCheck = document.getElementById('lifeHasLivingParentsCheck');
    var parentsBlock = document.getElementById('lifeParentsBlock');
    if (hasLivingParentsCheck) hasLivingParentsCheck.checked = state.lifeHasLivingParents;
    if (parentsBlock) parentsBlock.classList.toggle('hidden', !state.lifeHasLivingParents);
    renderLifeChildren();
    renderLifeParents();
    renderLifeMilestones();
    renderSeasonsDashboard();
    var dob = state.lifeDateOfBirth || '';
    var gender = state.lifeGender || 'male';
    ...
    if (clockEl) {
      ...
      } else clockEl.textContent = '— Months Lived | — Months Remaining';
    }
    if (scarcityEl) {
      if (summers != null && summers >= 0) {
        scarcityEl.textContent = 'You have roughly ' + summers + ' summers left. Make this year\'s Misogi count.';
      } else scarcityEl.textContent = 'Enter your date of birth to see your life in months.';
    }
    var wrapper = document.getElementById('lifeGridWrapper');
    if (!wrapper) return;
    var currentMonthIndex = monthsLivedVal;
    var maxYears = 120;
    var expectYears = getEffectiveLifeExpectancyYears(gender);
    var totalMonths = Math.min(expectYears * 12, maxYears * 12);
    var numRows = Math.ceil(totalMonths / 36);
    var cells = [];
    ...
    var progressBar = document.getElementById('lifeHorizonProgressBar');
    var progressPct = document.getElementById('lifeHorizonProgressPct');
  ...
  function switchView(mode) {
    ...
    var life = document.getElementById('lifeContent');
    var btnPlanner = document.getElementById('viewTogglePlanner');
    var btnAudit = document.getElementById('viewToggleAudit');
    var btnLife = document.getElementById('viewToggleLife');
    planner.classList.toggle('view-block-planner-off', mode !== 'planner');
    audit.classList.toggle('view-block-audit-visible', mode === 'audit');
    audit.classList.toggle('view-block-audit-off', mode === 'life');
    if (life) {
      life.classList.toggle('view-block-life-visible', mode === 'life');
    }
    ...
    if (mode === 'audit') renderAuditDashboard();
    if (mode === 'life') renderLifeDashboard();
  }
  ...
  function renderAuditDashboard() {
    ...
    var whatSetsApartInput = document.getElementById('auditMisogiWhatSetsApart');
  ...
  function init() {
    ...
    var viewToggleLife = document.getElementById('viewToggleLife');
    if (viewToggleLife) viewToggleLife.addEventListener('click', function () { switchView('life'); });
    ...
    var seasonsOnTheWayToggle = document.getElementById('seasonsOnTheWayToggle');
    if (seasonsOnTheWayToggle) {
      seasonsOnTheWayToggle.addEventListener('click', function () {
        var content = document.getElementById('seasonsOnTheWayContent');
        var expanded = this.getAttribute('aria-expanded') === 'true';
        this.setAttribute('aria-expanded', expanded ? 'false' : 'true');
        if (content) content.classList.toggle('hidden', expanded);
      });
