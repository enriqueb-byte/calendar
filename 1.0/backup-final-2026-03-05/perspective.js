...
/**
 * Calendar Planner — Perspective (perspective.js)
 * Contract: Expects window.CalendarPlanner with state, savePrefs, getCategory, escapeHtml.
 * Listeners attached: lifeFactsToggle, lifeDateOfBirth, lifeGender, lifeExpectancyOverrideCheck/Input,
 * lifeHasChildrenCheck, lifeAddChildBtn, lifeChildrenList, lifeHasLivingParentsCheck, lifeAddParentBtn,
 * lifeParentsList, seasonsInactiveToggle, seasonsOnTheWayToggle.
 */
...
function renderLifeDashboard() {
  if (!perspectiveDomCache.lifeContent) perspectiveDomCache.lifeContent = document.getElementById('lifeContent');
  var lifeContent = perspectiveDomCache.lifeContent;
...
if (window.CalendarPlanner) {
  window.CalendarPlanner.renderLifeDashboard = renderLifeDashboard;
  window.CalendarPlanner.perspectiveInit = perspectiveInit;
}
